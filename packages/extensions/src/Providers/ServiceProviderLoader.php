...
namespace OpenMeta\Extensions\Providers;
...
use OpenMeta\Extensions\Exceptions\ExtensionsException;
...
final class ServiceProviderLoader implements ServiceProviderLoaderInterface
{
    ...
    private function instantiate(string $providerClass): ServiceProviderInterface
    {
        if (! class_exists($providerClass)) {
            throw new ExtensionsException(sprintf('Service provider [%s] does not exist.', $providerClass));
        }

        $provider = new $providerClass();

        if (! $provider instanceof ServiceProviderInterface) {
            throw new ExtensionsException(sprintf(
